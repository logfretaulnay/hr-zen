...
-- Create admin user seed (this will be executed once)
DO $$
DECLARE
  admin_id UUID;
BEGIN
  -- Check if admin user already exists
  SELECT id INTO admin_id FROM auth.users WHERE email = 'admin@hrzen.demo';
  ...
  IF admin_id IS NULL THEN
    ...
    INSERT INTO public.profiles (user_id, email, name, role, job_title)
    VALUES (admin_id, 'admin@hrzen.demo', 'Admin HR-Zen', 'ADMIN', 'Administrateur Système');
  END IF;
END
$$;